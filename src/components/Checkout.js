...
class Checkout extends React.Component {
  ...
  checkout = async () => {
    ...
    if (this.validateResponse(errored, response, "checkout")) {
      if (response) {
        // Display a order successful message, update user's balance in localStorage and redirect to "/thanks" page
  ...
  async componentDidMount() {
    if (localStorage.getItem("username") && localStorage.getItem("token")) {
      ...
      message.error("Please log in before trying to checkout!!");
    ...
    return (
      <>
        ...
        <div className="checkout-container">
          <Row>
            {/* Cart should be shown on top of  Shipping and Pricing blocks in "xs" devices */}
