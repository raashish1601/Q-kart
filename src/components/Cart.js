...
export default class Cart extends React.Component {
  ...
  getCart = async () => {
    ...
    try {
      // Pass the OAuth token (check supported props in Cart class documentation) in the fetch API call as an Authorization header
      response = await (
  ...
  pushToCart = async (productId, qty, fromAddToCartButton) => {
    ...
    try {
      // Make an authenticated POST request to "/cart". JSON with properties - productId, qty are to be sent in the request body
      response = await (
        await fetch(`${config.endpoint}/cart`, {
          method: "POST",
          headers: new Headers({
  ...
  refreshCart = async () => {
    ...
    // If the user visits "/checkout" directly and cart is empty, display an error message and redirect to the "/products" page
  ...
  // Implement a lifecycle method to populate the Cart when page is loaded
  /**
  ...
  qtyChange = (value, item) => {
    this.pushToCart(item.productId, value);
  };
  // Implement getQuantityElement(). If props.checkout is not set, display a Input field.

  // Update getQuantityElement(). When displayed in the checkout page, display quantity of the item in cart (should be non-editable)
  /**
  ...
  getQuantityElement = (item) => {
    if (this.props.checkout) {
      return <InputNumber min={1} max={10} disabled defaultValue={item.qty} />;
    } else {
      return (
        <InputNumber
          min={1}
          ...
          onChange={(value) => this.qtyChange(value, item)}
        />
      );
    }
  };
  ...
  render() {
    return (
      ...
      >
              ...
              <Card className="cart-item" key={item.productId}>
                ...
                <div className="cart-parent">
                  ...
                  <div className="cart-item-qty">
                    {/*  Implement getQuantityElement() method */}
        ...
        {/* If props.checkout is not set, display a checkout button*/}
