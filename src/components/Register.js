...
class Register extends React.Component {
  ...
  validateInput = (event) => {
    ...
    if (this.state.username.length >= 6 && isValid) {
      isValid = true;
    } else {
      isValid = false;
      message.error("Username must be at least 6 characters long.");
    }
    if (this.state.username.length <= 32 && isValid) {
      isValid = true;
    } else {
      isValid = false;
      message.error("Username must not exceed max length of 32.");
    }
    ...
    if (this.state.password.length >= 6 && isValid) {
      isValid = true;
    } else {
      isValid = false;
      message.error("Password must be at least 6 characters long.");
    }
    if (this.state.password.length <= 32 && isValid) {
      isValid = true;
    } else {
      isValid = false;
      message.error("Username must not exceed max length of 32.");
  ...
  validateResponse = (errored, response) => {
    if (errored) {
      message.error("Network Issues.Try again Later!!");
      return false;
    }
    if (!response.success) {
  ...
  performAPICall = async () => {
    ...
    return this.validateResponse(errored, response);
  };
  ...
  register = async (event) => {
    ...
    if (isValid) {
      let apiCall = await this.performAPICall();
      if (apiCall) {
